...
public class Player : MonoBehaviour, IEquipmentObjectParent
{
  ...
  private void Update()
  {
    ...
    equipmentObjectCount = GetChildObjectCount(transform.Find(EQUIPMENT_OBJECT_HOLD_POINT));
  ...
  private void DropEquipment(Transform equipmentObjectDrop)
  {
    if (HasEquipmentObject())
    {
      // Player is carring something
      floorObject = GameObject.FindObjectOfType<FloorObject>();
      equipmentObjectDrop.transform.SetParent(floorObject.transform.Find(EQUIPMENT_OBJECT_DROP_POINT));
      equipmentObjectDrop.transform.position = equipmentObjectDropPoint.position;
    }
  }
  ...
  public void SetEquipmentObject(EquipmentObject equipmentObject)
  {
    ...
    Debug.Log("equipmentObjectList.length" + equipmentObjectList.Count);
  }
  ...
  {
    if (GetChildObjectCount(transform.Find(EQUIPMENT_OBJECT_HOLD_POINT)) == 0)
    {
